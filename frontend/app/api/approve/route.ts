...
export async function POST(req:Request){
  ...
  if(!pending.approval_required)return NextResponse.json({error:'only pending decision briefs can be approved'},{status:409});
  return NextResponse.json(runQuery(question,true),{headers:{'cache-control':'no-store','x-knowledgeops-approval':'explicit-human-action'}})
}
